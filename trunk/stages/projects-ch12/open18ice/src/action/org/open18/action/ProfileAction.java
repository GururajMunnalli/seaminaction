...
@Name("profileAction")
@Scope(ScopeType.CONVERSATION) // lets us redirect without losing @Out
public class ProfileAction implements Serializable {
	...
	@Factory("selectedGolfer")
	public void getSelectedGolfer() {
		try {
			loadProfile();
    ...
    public void loadProfile() throws ProfileNotFoundException {
        ...
        if (golferId != null && golferId > 0) {
			...
			// NOTE: disabling since this isn't working with natural conversations
			//try {
			//	selectedGolfer = (Golfer) entityManager.createQuery(
			//		"select g from Golfer g where g.username = ?1")
